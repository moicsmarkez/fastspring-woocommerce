/* globals jQuery, wc_geekguards_fastspring_params, fastspring, wc_checkout_params */
...
function setLoadingOn(msg = false) {
    msg = msg ? 'Tu pago está siendo procesado, por favor espera unos segundos. No cierres esta página' : 'por favor espera unos segundos. Verificando conexión segura.';
...
function getAjaxURL(endpoint) {
    return wc_geekguards_fastspring_params.ajax_url.toString().replace('%%endpoint%%', 'wc_fastspring_' + endpoint)
}
...
function requestPaymentCompletionUrl(data, cb) { // eslint-disable-line no-unused-vars
    setLoadingOn(true);
    data.security = wc_geekguards_fastspring_params.nonce.receipt
    jQuery.ajax({
        type: 'POST',
        dataType: 'json',
        ...
        success: function (result) {
            try {
                if (result.result === 'success') {
                    rogerthatUrl = result.receip_url;
                    cb(null, result)
...
function isFastSpringSelected() {
    return jQuery('.woocommerce-checkout input[name="payment_method"]:checked').attr('id') === 'payment_method_geekyguards_fastspring'
}
