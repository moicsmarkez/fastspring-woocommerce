...
class WC_Gateway_FastSpring_Handler {
  ...
  public function __construct() {
    $this->options = get_option('woocommerce_geekyguards_fastspring_settings', array());
    $this->init();
  }
  ...
  public function handle_webhook_request_order_completed($payload) {
    ...
    if ($order->payment_complete($payload->reference)) {
      $this->log(sprintf('Marking order ID %s as complete', $order->get_id()));
      $order->add_order_note(sprintf(__('FastSpring pago aprovado (ID: %1$s)', 'woocommerce'), $order->get_id()));
    } else {
      $this->log(sprintf('Ha fallado al marcar la orden ID %s como completa', $order->get_id()));
    }
  }
  ...
  public function handle_webhook_request_order_refunded($payload) {
    $order = $this->find_order_by_fastspring_tag($payload);
    $this->log(sprintf('Estado de orden ID %s como reembolsada', $order->get_id()));
  ...
  public function handle_webhook_request_subscription_canceled($payload) {
    $order = $this->find_order_by_fastspring_tag($payload);
    $this->log(sprintf('Estado de suscripción orden ID %s como cancelada', $order->get_id()));
  ...
  public function handle_webhook_request_subscription_activate($payload) {
    $order = $this->find_order_by_fastspring_tag($payload);
    $this->log(sprintf('Estado de suscripción orden ID %s como (re)activatada', $order->get_id()));
  ...
  public function handle_webhook_request_subscription_deactivate($payload) {
    $order = $this->find_order_by_fastspring_tag($payload);
    $this->log(sprintf('Estado de suscripción orden ID %s como desactivada', $order->get_id()));
